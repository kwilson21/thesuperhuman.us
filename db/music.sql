...
CREATE TABLE IF NOT EXISTS owner_request_audit_v2 (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  request_id TEXT NOT NULL REFERENCES owner_requests(id),
  action TEXT NOT NULL CHECK(action IN ('created','reviewed','resolved','reopened','withdrawn','note-updated','personal-data-deleted','payment-approved','booking-invoice-created','balance-invoice-created','booking-payment-updated','balance-payment-updated')),
  actor TEXT NOT NULL,
  note TEXT NOT NULL DEFAULT '',
  occurred_at TEXT NOT NULL
);
INSERT OR IGNORE INTO owner_request_audit_v2(id,request_id,action,actor,note,occurred_at)
