...
class CanonicalJournalStorageTests(unittest.TestCase):
    def test_uses_primary_checkout_for_a_linked_worktree(self):
        """A managed worktree must record in main's single private journal."""
        with tempfile.TemporaryDirectory() as temporary:
            primary = Path(temporary) / 'primary'
            worktree = Path(temporary) / 'worktree'
            with patch.object(journal, 'git_worktrees', return_value=[(primary, 'refs/heads/main'), (worktree, 'HEAD')]):
