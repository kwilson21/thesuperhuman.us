...
(async () => {
  ...
  try {
    ...
    const response = await page.goto(new URL('/building/personal-website', targetUrl).href, { waitUntil: 'domcontentloaded' });
    ...
    await proof.first().screenshot({ path: path.join(artifactDir, 'personal-website-journal-mobile.png') });
  } finally {
    await browser.close();
  }
