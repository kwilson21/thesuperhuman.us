...
All are ready for review. Footer and sitemap overlaps are resolved in the branches. The final tree matches the reviewed combined preview. This is preparation, not authorization to merge or publish.
...
Career and prelaunch changes, the Releases/Portfolio page framework, the revised services and intake, and the music infrastructure become available after the main deployment. Old News recording, release and portfolio example remain draft; production excludes their pages, media and demand submissions. The real A/B example is therefore present in development but withheld in production until the catalog is published.
...
Change the Old News recording, release and portfolio example from draft to public together in a reviewed publication change, then deploy and verify actual production routes, media, event recording and interest submissions. Do not treat a successful build as a live launch receipt. No automatic sales threshold or commerce flow is enabled.
