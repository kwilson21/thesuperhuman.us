...
September 10, 2026. The selected direction is implemented in the redesign PR stack; production deployment remains separate. This document records the shared model, with [content responsibilities](website-content-model.md), [selected studies](design-concepts/README.md), and [image QA](generated-image-qa.md) as its supporting references. See [delivery and launch gates](redesign-delivery.md) for integration status.
...
- The old headshot remains until the Audio replacement removes its final consumer.
  No replacement portrait is required by the selected compositions.
...
Work uses native disclosures for deeper evidence. About keeps a readable personal sequence with restrained, optional path motion. Daily and Threadline share ProjectLayout and ProjectTimeline while retaining custom explanations and source-driven updates. Writing uses a reading column, contents navigation and a native SVG explanation.

Audio stays within the personal site, reusing the accepted headphones, notebook, service icons and structured inquiry. Software and Audio service sheets share layout, tokens and print behavior. The website history uses the existing timeline with concise intention narratives, labelled studies and page comparisons; no quote extension or second timeline renderer is needed.

General contact, resume and Audio forms share client state handling while keeping their validation and delivery rules. New resume requests use the general variant; stored legacy requests retain their original PDF. Publication data, transport and private storage remain separate from curated page content.
