...
Status: agreed target model for the dependent redesign PRs. This documentation-only slice does not implement these pages or claim their verification. Source files, visual studies and implementation results arrive with the later slices.

Based on the [agreed direction](website-direction.md) and [original audit](audits/2026-09-09/website-audit.md).
...
Selected direction: a quiet studio on desktop and a continuous paper surface on mobile. Implementation and its verification belong to the dependent Home slice.
...
approval before delivery. See the approved request flow (reference accompanies the dependent implementation).
...
Approved desktop composition: Personal path, version 1 (reference accompanies the dependent implementation),
...
contact study (reference accompanies the dependent implementation) simplifies the
