...
if (form) {
  ...
  const payload = (data: FormData): Omit<IntakeInput, 'turnstileToken' | 'permission'> & {permission: boolean} => {
    ...
    return { service, direction, preferences, title: String(data.get('title') ?? '').trim(), fileLink: String(data.get('fileLink') ?? '').trim(), preserve: String(data.get('preserve') ?? '').trim(), referenceUrl: direction === 'judgment' ? '' : String(data.get('referenceUrl') ?? '').trim(), referenceNote: String(data.get('referenceNote') ?? '').trim(), name: String(data.get('name') ?? '').trim(), email: String(data.get('email') ?? '').trim(), permission: data.get('permission') === 'on' };
  };
  function renderService() {
    ...
    form!.querySelector<HTMLElement>('[data-reference-fields]')!.hidden = direction === 'judgment';
  }
  ...
  function showStep(index: number, focus = true) {
    ...
    if (focus) steps[index].querySelector<HTMLElement>('h2')!.focus();
  }
  function validateStep(): boolean {
    ...
    for (const field of fields) {
      ...
      if (control instanceof HTMLElement) {
        ...
        if (errors[field]) { control.setAttribute('aria-invalid','true'); first ??= control; }
      }
    }
