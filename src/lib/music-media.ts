...
export async function streamMusic(request: Request, bucket: R2Bucket, asset: { key: string; type: string }) {
  ...
  const headers = new Headers({ 'content-type': asset.type, 'accept-ranges': 'bytes', 'content-length': String(range?.length ?? head.size), 'cache-control': 'public, max-age=3600, s-maxage=3600', 'x-content-type-options': 'nosniff', 'content-disposition': 'inline' });
