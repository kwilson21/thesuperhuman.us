...
import { z } from 'zod';
import { clientPortalEnabled, discardUndeliveredCode, hashValue, issueClientCode, normalizeClientEmail } from '~/lib/audio-client-access';
...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  if (!env?.MUSIC_DB || !env.RATE_LIMIT || !env.TURNSTILE_SECRET_KEY || !env.AUDIO_CLIENT_CODE_KEY ||
  ...
  try {
    ...
    if (!ipLimit.allowed) return Response.json({ ok: false, error: 'Please wait a few minutes before asking for another code.' }, { status: 429 });
