...
- Every PR gets a code review before it merges. If Greptile has not reviewed the PR's current head, run independent review subagents on the diff instead (for example, one for correctness and security, one for UI, copy and the rules in this file). Verify each finding against the code, fix the real ones, push, and review the new head again. Repeat until a review pass finds nothing, then merge once CI is green. Findings that need a product or design decision go to Kazon instead of being fixed silently
