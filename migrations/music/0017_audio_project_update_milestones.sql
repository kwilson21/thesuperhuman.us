...
-- Each note was written in one batch with its audit row, at the same instant. Label a
-- note only when that pairing is unambiguous: one plain note and one matching audit row.
UPDATE audio_project_updates SET milestone='revision_started'
  WHERE milestone IS NULL AND kind='progress' AND file_id IS NULL
    AND (SELECT COUNT(*) FROM audio_project_updates u WHERE u.request_id=audio_project_updates.request_id
      AND u.created_at=audio_project_updates.created_at AND u.kind='progress' AND u.file_id IS NULL)=1
